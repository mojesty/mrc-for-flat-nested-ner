...
def entity_span_loss_fct(start_logits, end_logits, span_embedding=None, span_match=None):
    ...
    if span_embedding is None:
        span_loss = span_loss_fct(start_logits.view(-1, 2), start_positions.view(-1)) + span_loss_fct(
            start_logits.view(-1, 2), start_positions.view(-1))
    else:
        entity_span_logits = torch.concat(start_logits.view(-1, 2), end_logits.view(-1, 2))
